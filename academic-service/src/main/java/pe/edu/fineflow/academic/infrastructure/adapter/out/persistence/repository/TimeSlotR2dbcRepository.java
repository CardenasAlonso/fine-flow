...
public interface TimeSlotR2dbcRepository extends R2dbcRepository<TimeSlotEntity, String> {
    ...
    @Query("SELECT * FROM TIME_SLOTS WHERE id = :id AND school_id = :schoolId")
    Mono<TimeSlotEntity> findByIdAndSchoolId(String id, String schoolId);

    @Query("DELETE FROM TIME_SLOTS WHERE id = :id AND school_id = :schoolId")

...
public class StudentController {
    ...
    public Flux<StudentDto.Response> findBySection(@PathVariable String sectionId) {
        return useCase.findAllBySection(sectionId).map(this::toResponse);
    }
    ...
    public Mono<StudentDto.Response> create(@RequestBody StudentDto.Create request) {
        return useCase.create(mapper.toDomain(request)).map(this::toResponse);
    }

    @PutMapping("/{id}")
    @PreAuthorize("hasAnyRole('ADMIN','COORDINATOR')")
    public Mono<StudentDto.Response> update(
            @PathVariable String id, @RequestBody StudentDto.Update request) {
    ...
    public Flux<StudentDto.Response> search(@RequestParam String q) {
        return useCase.search(q).map(this::toResponse);
    }

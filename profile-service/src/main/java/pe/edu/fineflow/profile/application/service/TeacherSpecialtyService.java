...
@Service
public class TeacherSpecialtyService implements ManageTeacherSpecialtyUseCase {

    private final TeacherSpecialtyRepositoryPort repo;

    public TeacherSpecialtyService(TeacherSpecialtyRepositoryPort repo) {
        this.repo = repo;
    }
    ...
    public Mono<Void> remove(String id) {
        return TenantContext.getSchoolId()
                .flatMap(
                        sid -> {
                            TeacherSpecialty dummy = new TeacherSpecialty();
                            dummy.setId(id);
                            dummy.setSchoolId(sid);
                            return repo.delete(id).onErrorResume(
                                    e -> Mono.error(new ResourceNotFoundException("TeacherSpecialty", id)));
                        });
    }

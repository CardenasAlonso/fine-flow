...
class ReportRequestServiceTest {
    ...
    void setUp() {
        ...
        service = new ReportRequestService(repo, generator, storage);
    }
    ...
    void requestCreatesPendingJobForPrincipal() {
        ...
        when(repo.updateStatus(anyString(), anyString(), anyInt(), nullable(String.class)))
                .thenReturn(Mono.empty());

        StepVerifier.create(service.request("REPORT_CARDS", "PDF", "{}")
                        .contextWrite(ctx -> ctx.put(TenantContext.PRINCIPAL_KEY, principal())))
                .assertNext(job -> {
                    org.assertj.core.api.Assertions.assertThat(job.getStatus()).isEqualTo("PENDING");

...
@Slf4j
@Service
public class BlockchainUseCaseImpl implements BlockchainUseCase {

    private final BlockchainRepositoryPort repo;
    private final EventBus eventBus;

    public BlockchainUseCaseImpl(BlockchainRepositoryPort repo, EventBus eventBus) {
        this.repo = repo;
    ...
    public Mono<BlockchainBlock> appendBlock(
        ...
        return repo.findLatestBySchoolId(schoolId)
                .defaultIfEmpty(BlockchainBlock.genesis(schoolId, triggeredBy))
                .flatMap(
                        prev -> {
    ...
    public Mono<Boolean> verifyChain(String schoolId) {
        return repo.findAllBySchoolId(schoolId)
                .reduce(new VerifyState(true, null), (state, block) -> {
                    ...
                    if (!block.hasValidChainLink(state.prev.getHash())) {
